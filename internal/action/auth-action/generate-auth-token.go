...
func GenerateAuthToken(user *model.User) (string, string, *time.Time, *time.Time, error) {
	tokenDuration := time.Duration(viper.GetInt(config.JwtExpirationHours)) * time.Hour
	...
	refreshDuration := time.Duration(viper.GetInt(config.JwtRefreshExpirationDays)) * 24 * time.Hour
